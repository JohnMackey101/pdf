...
pdf_anonymizer.py
-----------------
Detects and masks PII in a PDF, allowing the user approve each redaction
before writing masked text back into the document. 

Once pre-requisites are met use the following command:
    python pdf_anonymizer.py                    # uses default config
...
def main() -> None:
    ...
    # Ask for PDF path
    print("\n" + "=" * 60)
    print("         PDF Anonymizer — Interactive Redaction Tool")
    print("=" * 60)
    ...
    if not approved:
        print("No redactions approved. Exiting without saving.")
        sys.exit(0)
